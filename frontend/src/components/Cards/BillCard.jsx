...
const BillCard = ({ billflag,setbillflag,user,BillData,thememode }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const editBill = async () => {
      try {
        const res = await axios.put(`https://expense-tracker-paisa-vasooli.onrender.com/api/bills/editBill/${BillData._id}`, { BillInput });
  ...
  const handleDelete = (id) => {
    const delBill = async (id) => {
      try {
        const res = await axios.delete(`https://expense-tracker-paisa-vasooli.onrender.com/api/bills/deleteBill/${id}`);
        console.log(res.data);
        setbillflag((prev)=>!(prev))
      } catch (err) {
        console.log(err);
