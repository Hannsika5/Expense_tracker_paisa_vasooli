...
function Vault({ thememode, toggle, user }) {
  ...
  const uploadFile = () => {
    ...
    uploadBytes(fileRef, fileUpload).then(() => {
      getDownloadURL(fileRef).then((url) => {
        ...
        const addUrl = async () => {
          try {
            const res = await axios.post(`https://expense-tracker-paisa-vasooli.onrender.com/api/user/addUrl/${user._id}`, { url, fileName });
  ...
  useEffect(() => {
    const getFiles = async () => {
      try {
        const res = await axios.get(`https://expense-tracker-paisa-vasooli.onrender.com/api/user/getUrls/${user._id}`);
